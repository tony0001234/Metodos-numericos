...
int main(){//creo una funcion vacia o proceso, vacia ya que no quiero que devuelva nada solo se ejecute, el proceso imprimira el menu para que se vea bonito.
    ...
    float puntoZ, p, valor = 0;//declaro mis variables para mostrar los resultados y la de puntoZ para pedir el punto en el que se valida la funcion
    ...
    datoFXI = valoresXY[0][1];
    for (int i = 0; i < n-1; i++)
    {
        p = 1;
        for (int j = 0; j <= i; j++)
        {
            p = p * (datoXI - valoresXY[j][0]);
        }
        datoFXI = datoFXI + tabla[i][i] * p;
    }
    cout << datoFXI<<endl;
    ...
    for (int i = 0; i <= m-1; i++)
    {
        tabla[i][0] = (valoresXY[i+1][1] - valoresXY[i][1]) / (valoresXY[i+1][0] - valoresXY[i][0]);
    }
    for (int j = 0; j <= m-1; j++)
    {
        for (int i = j; i <= m-1; i++)
        {
            tabla[i][j] = (tabla[i][j-1] - tabla[i-1][j-1]) / (valoresXY[i+1][0] - valoresXY[i - j][0]);
            cout<< "Valor A" << i << ": "<< tabla[i][j] <<endl;
        }
    }
